...
class ModelRunner(GPUModelRunnerBase[ModelInputForGPUWithSamplingMetadata]):
    ...
    def execute_model(
        ...
        # logger.info("attn state fwd begin")

        self.attn_state.begin_forward(model_input)

        # logger.info("attn state fwd end")
        ...
        # logger.info("check if need recv kv cache")
        if self.need_recv_kv(model_input, kv_caches):
            # logger.info("recieve kv cache")
            (
        ...
        # logger.info(f"start model forward,  {bypass_model_exec=}")
        if (
        ...
        if not bypass_model_exec:
            with set_forward_context(
                ...
                # logger.info(f"start call model_executable, {type(model_executable)=}")
                # logger.info(
                #     f"{model_input.input_tokens=}, {model_input.input_positions}, {kv_caches=}, {model_input.attn_metadata=}, {intermediate_tensors=}"
                # )
                ...
                # logger.info(f"finsih call model_executable")
        ...
        # logger.info(f"compute logits {self.is_driver_worker=}")

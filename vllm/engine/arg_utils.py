...
class EngineArgs:
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        parallel_group.add_argument(
            "--master-addr", **parallel_kwargs["master_addr"]
        )
        parallel_group.add_argument(
            "--master-port", **parallel_kwargs["master_port"]
        )
        parallel_group.add_argument(
            "--nnodes", **parallel_kwargs["nnodes"]
        )
        parallel_group.add_argument(
            "--node-rank", **parallel_kwargs["node_rank"]
        )
        parallel_group.add_argument(
    ...
    def create_engine_config(
        ...
        assert (
            self.data_parallel_backend == "mp" or not self.nnodes > 1
        ...
        if self.nnodes > 1:
            ...
            assert self.node_rank < self.nnodes, (
                f"node_rank={self.node_rank} must be less than "
                f"nnodes={self.nnodes}."
            )
            ...
            if self.data_parallel_size > 1 and self.data_parallel_external_lb:
                ...
                logger.info(
                    "Inferred data_parallel_rank %d from node_rank"
                    " %d for external lb",

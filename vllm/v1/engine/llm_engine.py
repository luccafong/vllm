...
import torch
import math
class InferenceDPSampler:
    def __init__(self, batch_size: int, dp_group_size: int):
        self.shuffle = True
        self.seed = 123
        self.batch_size = batch_size
        self.dp_group_size = dp_group_size
        self.num_samples = math.ceil(self.batch_size / self.dp_group_size)
    ...
    def __iter__(self):
        if self.shuffle:
            ...
            indices = torch.randperm(batch_size, generator=g).tolist()  # type: ignore[arg-type]
        else:
            indices = list(range(batch_size))  # type: ignore[arg-type]

            # add extra samples to make it evenly divisible
        padding_size = self.num_samples * self.dp_group_size -  self.batch_size

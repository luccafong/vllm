...
class MultiprocDistributedExecutor(MultiprocExecutor):
    ...
    def init_workers(self, unready_workers: list[UnreadyWorkerProcHandle]) -> None:
        ...
        return super().init_workers(unready_workers)    
    ...
    def init_response_mqs(self) -> None:
        ...
        self.rpc_response_mqs = self.workers[0].rpc_response_mqs
    def get_message_queue(self, unique_reply_rank: Optional[int] = None) -> List[MessageQueue]:
        message_queues = []
...
class DistributedWorkerProcHandle(WorkerProcHandle):
    ...
    def from_unready_handle(
            ...
            worker_response_mq: Optional[MessageQueue], **kwargs) -> "DistributedWorkerProcHandle":
...
class DistrbutedWorkerProc(WorkerProc):
    ...
    def init_message_queues(self, input_shm_handle: Handle, vllm_config: VllmConfig) -> None:
        ...
        if node_size == 1:
            ...
            self.worker_response_mq, self.rpc_response_handles = get_world_group().create_single_reader_mq_broadcasters(
                reader_rank=0)
    @classmethod
    def wait_for_response_handle_ready(cls, handles: dict[str, Any], proc_handle: UnreadyWorkerProcHandle) -> WorkerProcHandle:
        ...
        remote_response_mqs = [MessageQueue.create_from_handle(
            response, -1) if response.remote_subscribe_addr is not None else None for response in remote_response_handles]
        return DistributedWorkerProcHandle.from_unready_handle(proc_handle, worker_response_mq, rpc_response_mqs = remote_response_mqs)

    @classmethod
    def get_ready_proc_handles(cls, worker: "WorkerProc") -> dict[str, Any]:
        assert isinstance(worker, DistrbutedWorkerProc), "worker must be DistrbutedWorkerProc"
        return {
                "status":

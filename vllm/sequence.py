...
class HiddenStates(msgspec.Struct, array_like=True,
    ...
    def update(self,
        ...
        if self.hidden_states_seq_indices is not None:
            ...
            # assume new updated are hidden states from prefill which is always length of 1
            new_seq_indices = torch.tensor(updated_indices, device=self.hidden_states_seq_indices.device) 
    ...
    def prune(self,
        ...
        if seq_ids != self._seq_ids:
            ...
            if self.hidden_states_seq_indices is not None:
                target_indices_tensor = torch.tensor(index, device=self.hidden_states_seq_indices.device)
                index = (self.hidden_states_seq_indices[..., None] == target_indices_tensor).any(dim=-1)
